...
public struct FedNoiseKeyPair: @unchecked Sendable, Equatable {
...
public struct FedNoiseHandshakeResult: @unchecked Sendable {
    public let transport: FedNoiseTransport
    public let handshakeHash: Data

    fileprivate init(transport: FedNoiseTransport, handshakeHash: Data) {
        self.transport = transport
        self.handshakeHash = handshakeHash
    }
}

final class FedNoiseSymmetricState: @unchecked Sendable {
...
public final class FedNoiseIKInitiator: @unchecked Sendable {
    ...
    private var result: FedNoiseHandshakeResult?
    ...
    public func readMessage2(_ message: Data) throws -> FedNoiseHandshakeResult {
        ...
        let transport = try FedNoiseTransport(sendKey: split.initiatorToResponder, receiveKey: split.responderToInitiator)
        let completed = FedNoiseHandshakeResult(transport: transport, handshakeHash: symmetric.handshakeHash)
        result = completed
        handshakeComplete = true
        return completed
    }

    public var transport: FedNoiseTransport? { result?.transport }
...
public final class FedNoiseIKResponder: @unchecked Sendable {
    private let staticKey: FedNoiseKeyPair
    ...
    public func readMessage1(_ message: Data) throws -> Data {
        ...
        let split = symmetric.split()
        let transport = try FedNoiseTransport(sendKey: split.responderToInitiator, receiveKey: split.initiatorToResponder)
        result = FedNoiseHandshakeResult(transport: transport, handshakeHash: symmetric.handshakeHash)
        handshakeComplete = true
        return response
    }
...
public final class FedNoiseTransport: @unchecked Sendable {
    ...
    private(set) public var nextSendNonce: UInt64 = 0
    private(set) public var nextReceiveNonce: UInt64 = 0
    private(set) public var isClosed = false
    private(set) public var rekeyRequired = false

    public init(sendKey: Data, receiveKey: Data, nextSendNonce: UInt64 = 0, nextReceiveNonce: UInt64 = 0) throws {
        guard sendKey.count == 32, receiveKey.count == 32 else { throw FedNoiseError.invalidKeyLength }
        self.sendKey = sendKey
        self.receiveKey = receiveKey
        self.nextSendNonce = nextSendNonce
        self.nextReceiveNonce = nextReceiveNonce
        self.rekeyRequired = nextSendNonce >= Self.rekeyMessageCount
    }

    public func encrypt(_ plaintext: Data) throws -> Data {
        guard !isClosed else { throw FedNoiseError.transportClosed }
        guard nextSendNonce <= Self.hardBackstopNonce else {
            isClosed = true
        ...
        nextSendNonce += 1
        if nextSendNonce >= Self.rekeyMessageCount { rekeyRequired = true }
        if nextSendNonce > Self.hardBackstopNonce { isClosed = true }
        return ciphertext
    }

    public func decrypt(_ ciphertext: Data) throws -> Data {
        guard !isClosed else { throw FedNoiseError.transportClosed }
        guard nextReceiveNonce <= Self.hardBackstopNonce else {
            isClosed = true
            throw FedNoiseError.hardBackstop
        }
        let plaintext = try FedNoiseChaChaPoly.open(
        ...
        nextReceiveNonce += 1
        if nextReceiveNonce > Self.hardBackstopNonce { isClosed = true }
        return plaintext
    }

    public func encodeTransportRecord(_ plaintext: Data) throws -> Data {
        try FedOuterRecordCodec.encode(try encrypt(plaintext))
    }

    public func decodeTransportRecord(_ outerRecord: Data) throws -> Data {
        try decrypt(FedOuterRecordCodec.decodeTCPRecord(outerRecord))
    }

    /// Testing-only: set the next outbound transport nonce to simulate a session
    /// approaching the maximum safe nonce. Production advances it only on send.
    public func setNextSendNonceForTesting(_ nonce: UInt64) {
        nextSendNonce = nonce
        rekeyRequired = nonce >= Self.rekeyMessageCount
    }
}
