...
| Subc-generated routing/control error | `ERROR` on the relevant `channel`/`corr` | `passive`; `BINARY=0`, `LAST=0` | No | No | Body is canonical JSON `ErrorBody { code: string, message: string, detail?: object }` for all subc-generated errors, including control-plane errors and unknown-channel routing failures. |
