...
- **alfonso-tui (CKTUI)** — Ratatui TUI harness driving broca sessions over
  subc. Renamed from brocatui 2026-08-11 (app-repo naming convention:
  alfonso-ios, alfonso-desktop, alfonso-tui). GitHub: `cortexkit/alfonso-tui`
  (private), CI on Blacksmith (ubuntu + windows + marker sweep). Binary is
  still `ck-brocatui` and the store path `~/.local/share/cortexkit/brocatui/`
  — renaming those is a product decision with a store migration, deliberately
  not folded into the repo move (37 session records key on the old path).
