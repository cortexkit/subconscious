...
| WHEN THE REFRESH INTERVAL IS SHORTER THAN THE OPERATION IT REFRESHES, "PERIODIC" BECOMES "CONTINUOUS BY ARITHMETIC": a 30m cadence on a ~2.5h walk means the walk always runs -- a fixed interval chosen when the operation was fast, silently lapped by growth, no alarm because both numbers are individually healthy (the 3-hour-ceiling class as a scheduling constant). Check which semantics the code implements -- fixed-period vs completion-plus-rest -- because they differ exactly when the operation is slow, which is when it matters. A permanent background operation also re-baselines every OTHER measurement on the shared resource. | engram's GC_PREVIEW_COMPLETED_WALK_INTERVAL: total_elapsed 33min on a 30min refresh; the DO queue-behind-the-walk signature is now the permanent background of every measurement on the account. |
