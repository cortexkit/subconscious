...
| sqlite's integrity_check on a 0-byte file returns Ok("ok") -- the tool answers "no corruption" and an empty file is uncorrupted, so the deepest capture gate CERTIFIES nothing-at-all as a valid backup, forever, with a legitimate manifest entry. Fix at the capture (refuse a declared database with no schema: the honest states are unmeasured and measured-and-empty, never one), not at the alarm that surfaced it -- the alarm was the symptom layer. | engram tested rather than reasoned: 0-byte db opens read-only, integrity Ok, 0 tables. Cost asymmetry: refusing costs one skipped capture; proceeding costs a false belief of protection. Check-narrower-than-its-use at the bottom of the stack. |
