...
| A comment describing a nearby CORRECT mechanism launders an adjacent WRONG one: read the call site, not the comment. An expensive computation can masquerade as an invariant while the real invariant sits six lines away doing the work for free. | engram recomputed an O(inventory) whole-inventory digest on all ~700 pages; its comment said it stopped a dropped page -- true of the PAGE-chain digest beside it, false of itself. The whole-inventory digest had one consumer (server recompute-and-compare at plan submit): an echo token, needed once, sent 700 times. Fix removed work (send on terminal page only), did not add a chain. |
