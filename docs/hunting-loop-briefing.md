...
| WHEN A DEPENDENCY BUMP ADDS A REFUSAL SURFACE, LAND IT AGAINST INPUT THAT CANNOT REFUSE BEFORE POINTING IT AT INPUT THAT CAN: bump-and-cutover as one change makes a genuine upstream malformation indistinguishable from a cutover bug on the first failure. ASTRO's split: 0.2.0 against the 23-day-static hand-staged file first (same bytes parsed 5,297 rows every tick -- refusal provably unreachable), THEN the automated-source cutover separately. One variable per change, applied to a version bump. | The loud-failure path and the untrusted-input path must not go live in the same instant. |
