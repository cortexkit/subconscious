...
- **Wire crate bump train:** after `CI` succeeds on `master`, the workflow publishes unpublished `subc-protocol`, `subc-transport`, `subc-control`, and `subc-client-rs` versions in dependency order from the exact green commit. Each successful publish writes an annotated `<crate>-v<version>` tag at that commit as the release record. [subconscious/.github/workflows/publish-on-bump.yml] [subconscious/.github/workflows/publish-crate.yml] [subconscious/scripts/release/unpublished-wire-crates.sh]
