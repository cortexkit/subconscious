...
corpus carries exactly these names plus at least one positive named `minimal`,
and a consumer MAY diff the set it runs against the set it received in both
directions — refusing a corpus that omits a required name, and refusing one that
carries a name it does not run.
