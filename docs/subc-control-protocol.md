...
**subc UNDERSTANDS (closed set):** `hello`/`hello_ack`; `PING`/`PONG`/`GOODBYE`/`CANCEL` (frames); `server.describe`; `catalog.list`; `route.open`→`route.bind`; `route.poll`(status|liveness); `supervisor.*`; (future) `lease.*`, `config.*` as RAW tier get/put/changed transport ONLY; module `route.status` push (opaque string, cached verbatim).
