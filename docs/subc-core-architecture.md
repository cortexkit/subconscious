...
- **Connection file = discovery + key.** subc atomically publishes (temp + rename, owner-only perms pre-applied) a per-user connection file at a well-known path: `{schema, endpoints:[{host,port}], key (≥256-bit random), daemon_id (random per start), pid (advisory only), daemon_ver}`. Clients read it to discover the endpoint **and** the key; readers never observe a partial file (normative: atomic temp+rename + owner-only `0600`/user-ACL applied *before* publish — a truncated/partial-key read is impossible by construction).
