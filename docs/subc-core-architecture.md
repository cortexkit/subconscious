...
### 4.10 Spawn-mode signal & connect-failure contract (subc → spawned child)

How subc tells a process it spawned "you are running under me." Co-signed with AFT; applies to **any** subc-spawned child (AFT, the MCP gateway `subc-mcp`, future modules), not AFT-specifically.

- **One signal: `SUBC_SOCKET`.** subc's Supervisor sets a single env var on the child at spawn: `SUBC_SOCKET=<uds-path>`. **Presence = subc mode** (the child enables its under-subc paths: e.g. AFT picks shared-storage + multi-root `ProjectActor` demux); **value = the endpoint** the child connects *out* to, then sends HELLO/manifest unchanged. **Absence = standalone.** One branch carries both *mode* and *where* — no separate boolean, no implicit default. The signal is pre-HELLO bootstrap; the HELLO handshake (§4.2) is unchanged. So every subc-mode codepath ships **dormant in standalone by construction** — activated only when the var is present.
- **Bind-before-spawn (no race).** subc binds and is accepting on the socket **before** the Supervisor spawns the child with `SUBC_SOCKET` set (the daemon bootstrap binds/listens, *then* `Supervisor::spawn`). So in correct operation `SUBC_SOCKET`-present ⟹ socket-listening; a spawn-before-listening race cannot occur.
- **Present-but-unconnectable ⟹ FAIL LOUD, never a silent downgrade.** A set-but-dead `SUBC_SOCKET` is a *broken invariant* (only subc sets it, only after binding). The child errors and refuses to start — it does **not** silently run standalone. Silent fallback there is precisely the split-brain the daemon exists to kill (subc believes it owns the root while the child quietly ran standalone with its own watcher/index/storage → invisible duplication + divergent state, the §6/4.3 two-writer hazard). A **short bounded connect-retry** (~100–500 ms, transient accept-hiccup insurance) is permitted, then fail-loud — never a topology downgrade.
- **The child never decides topology — the harness-client does.** A subc-spawned child obeys `SUBC_SOCKET` and fails loud if it is broken; subc's Supervisor handles the exit. **All** topology/fallback decisions belong to the harness-side subc-client: *daemon-absent* (no live subc at the discovery path) → it falls back to in-process standalone execution (Story 1.2), spawning the standalone child **without** `SUBC_SOCKET`; *daemon-died mid-session* → it reconciles and falls back (Story 4.3, no two-writer). "Silent fallback to standalone" exists, but it is the harness-client reacting to subc-**absence**, never a child reacting to a dead `SUBC_SOCKET`.
- **Stale-env closure is structural, in the subc-client core.** `SUBC_SOCKET` is meaningful only on the direct subc→child spawn: set fresh by subc's Supervisor, and **explicitly cleared** by the subc-client core's standalone-spawn helper when it launches an in-process/standalone child (never trusted via ambient inheritance). The clearing lives in the **generic core**, not per-module adapters — so every module (AFT, MC) inherits it and none can forget it and reopen the hole. A stale exported `SUBC_SOCKET` from a dead subc therefore cannot activate subc-mode in a standalone child; fail-loud is the backstop if one ever survives.
