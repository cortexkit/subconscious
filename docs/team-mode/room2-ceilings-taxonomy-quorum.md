...
- **Step-up identity (contract-pinned, CKCRED [#20]; r4 adds the caller-binding pin the gate demanded)**: purpose `raise_ceiling`, audience `cortexkit-account:raise-ceiling`, 300s TTL, single-use with consume-with-op discipline (jti recorded by the mutation batch it authorizes, same family as `orgs.create_attestation_jti`). The attestation names the acting ADMIN — realized in the concrete wire shape as the `account_id`/`sub` claim carrying the acting admin's account (r5 mapping pin: wherever this contract says `attestation.admin`, the CKCRED wire artifact carries it as `account_id`; one fact, two spellings, normatively equivalent) — and the S1 predicate of the consuming batch REQUIRES that claim == the authenticated caller of the mutation AND attestation org == the mutation's org — in the same guarded transaction that consumes the jti. This closes first-use substitution (a stolen attestation is unusable by any other caller or org), which single-use alone does not: single-use stops replay, caller/org binding stops substitution. The raise target stays in the mutation body — with caller+org binding and atomic consume, target-binding in the mint would add one fresh login per raise and no security.
...
- The compiler lives **org-side** (CKCRED schema home). It compiles role → `federation_exposure` set: a closed, deny-unknown-fields list of `ExposeEntry` values, each EXACTLY-ONE-OF `{module, tool}` or `{module, operation}` (r5 normalization — the earlier `{module_id, tools:[], operations:[]}` sketch is superseded; the normative shape IS fed's existing profile `expose` structure verbatim) (`ExposeEntry::Tool` / `ExposeEntry::Operation`, crates/fed-core/src/profile.rs). FED owns this shape; the org-side compiler produces conforming sets; a shape change is a fed-schema amendment the compiler follows (FED [#21]).
...
### 4.1b Denial short-circuit (r5, ALF [#28] chair-adopted [#29], gate-cleared)

Any qualifying subject's DENIAL resolves the ask denied immediately — quorum gates approval, not denial. One qualified "no" stops an above-ceiling action; requiring deny-quorum would make blocking a dangerous action harder than performing a safe one. Denial eligibility uses the same full-conjunction rule as approval eligibility (§4.2).
...
- **FLIP-TIME REVALIDATION (r4 — closes the shrinkage gap the gate caught)**: epoch-bump events (kick, delegation revocation) kill the ask outright, as Room 1 already guarantees. Ceiling/role/ACL changes that arrive as ordinary bundle_version bumps do NOT kill the ask; instead, the CAS-flip transaction (the Nth qualifying approval) REVALIDATES every counted approval against the CURRENT snapshot: an approver no longer eligible under the current bundle has their approval discarded (typed annotation `approval_lapsed`), and the flip proceeds only if N qualifying approvals survive revalidation. "Current" means the latest DecisionView snapshot OBSERVED AT FLIP START (same resolution path as serve admission, one snapshot per transaction, no re-poll and no second read) — a bundle change landing after that observation is the ordinary poll-latency class every Room-1 surface accepts (ALF [#31] precision, pinned so vectors test observed-snapshot semantics, not a stronger forced-re-poll reading). MONOTONICITY GUARD (ALF [#37]): each counted approval annotation records the bundle_version it was cast under; the flip records the bundle_version it revalidated against and REFUSES (stays parked, no lapse annotations) if that version is older than the max over counted approvals' cast-under versions — bundle versions only advance in a healthy system, so the guard is free in the normal case and makes snapshot-resolution regression loud instead of quietly lapsing approvals against stale facts. The frozen set still bounds the electorate above (no expansion, ever); revalidation bounds it below (no lapsed authority counts at the moment authority is exercised). One added read in the same transaction the CAS already runs, same snapshot discipline.
...
- Dispatch envelope (Zone 1): `action_stamp` `{tier_class, credential, financial}` plus sibling envelope field `taxonomy_version` — BOTH stamped by the same Zone-1 writer in the same admission step, never client-supplied; client-supplied values reject pre-parse by the same unrepresentability discipline as target_agent. (r5 normalization to ALF's implementing shape: the stamp struct is pure compiler output, the version is envelope metadata; the skew check reads both, and unrepresentability is unchanged because one writer stamps both.)
...
## A1 Amendments (post-freeze)

### A1-001 — Mint the taxonomy identifier `ck-action-severity/v1`

Status: posted by chair (SUBC); ALF co-signs as taxonomy-compiler owner; CKCRED + FED confirm (they carry `taxonomy_version` in the A2 claim / bundle artifacts). Binding is deterministic on post — consumers may cite it immediately; the all-seat confirm ratifies, it does not gate the binding.

Motivation: the contract pins a `taxonomy_version` string (§1.2 skew check, §2.3 A2 claim, §5 A2/envelope additions) as a "contract-pinned constant" but never mints the concrete token. A module citing "the fleet action-severity taxonomy" without a versioned identifier is a genuine scope-honesty gap (surfaced by the ck-cerebellum browser-plane spec's Athena panel, which correctly flagged that per-capability severity floors cannot cite an unversioned taxonomy). Every future consumer (cerebellum, wernicke's chat-gateway authority, motor/computer-use, any module with reversibility ceilings) will reference this taxonomy, so an unversioned taxonomy is a latent scope-honesty gap in each of them. Minting the identifier once, here, closes the class.

Amendment: the `taxonomy_version` contract-pinned constant has the value **`ck-action-severity/v1`**. Its normative definition IS §1 (taxonomy1: the ladder `OBSERVE < COMPOSE < MUTATE_WORKSPACE < MUTATE_DURABLE < PUBLISH_EXTERNAL`, ordinals 1..5, plus the orthogonal domain flags `{credential, financial}`) of THIS document at commit `d3cf7487`. The identifier binds `token -> (doc section §1, commit d3cf7487)`; nothing else changes. Wherever the contract says the constant is "stamped from a contract-pinned constant" (§1.2, §5 A2 claim, §5 dispatch envelope), that constant's value is `ck-action-severity/v1`.

Revision process: a future taxonomy change (adding/reordering a ladder class, adding a flag) is a new A1 amendment minting `ck-action-severity/v2` bound to the amended §1 at its new commit; the signer constant changes to `/v2` in the same deploy class as the compiler (§1.2's skew-deploy rule), and the skew check (§1.2) refuses to compare a `/v1` stamp against `/v2` claims — fail-closed on skew, as already specified. The identifier is a pure indirection over the existing section+commit binding; it adds no new storage and no new skew surface (the stamp already carries the version string).

Consumer guidance (per ALF, for modules pinning per-capability severity floors): map each capability to a LADDER tier and carry `{credential, financial}` as FLAGS rather than inflating the tier — a capability that can touch a login/payment surface stamps the flag, it does not jump to tier 5. This keeps a module's floors composable with the §2.5 ceiling gate's field-for-field (ladder-by-ordinal, flag-by-flag) check instead of inventing a parallel severity axis. Example: `browser.navigate` (pure fetch/observe) floors at OBSERVE; `browser.click` on an arbitrary page that can submit a form reaches MUTATE_DURABLE-equivalent territory; either carrying `{credential: true}` when the target is a login surface.

Fixture migration (completes the amendment — the minted value and the conformance fixtures MUST agree, or an implementer building against the fixtures hardcodes the stale token): every Room-2 conformance fixture carrying `taxonomy_version`/`stamp_version`/`view_version` migrates the literal working-name token `taxonomy1` → `ck-action-severity/v1`. The skew-test vectors (which need two DISTINCT tokens to prove the §1.2 fail-closed skew refusal) migrate the placeholder `taxonomy2` → `ck-action-severity/v2` — the real future-revision identifier, which is more faithful than the abstract placeholder. Artifact custody is unchanged: each carrier regenerates its own canonical (CKCRED the A2/ceiling fixtures, ALF the compiler/gate conformance vectors), and the chair (SUBC) syncs the vendored mirrors under `docs/team-mode/` from the pushed canonicals. This is a pure literal-token substitution; no vector's expected outcome changes (a match stays a match, the skew case stays a refusal), so it is byte-mechanical, not a semantic revision.
